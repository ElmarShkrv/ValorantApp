package com.chiore.valorantapp.repository

import com.chiore.valorantapp.api.ValorantApi
import com.chiore.valorantapp.data.model.maps.MapDetailResponse
import com.chiore.valorantapp.util.Resource
import javax.inject.Inject

class MapsDetailsRepository @Inject constructor(
    private val valorantApi: ValorantApi,
) {
    suspend fun mapDetails(mapUuid: String): Resource<MapDetailResponse> {
        return try {
            val response = valorantApi.getMapByUuid(mapUuid)
            if (response.isSuccessful) {
                response.body()?.let {
                    return Resource.Success(it)
                } ?: Resource.Error("Response is null")
            } else {
                Resource.Error("Response is not successful")
            }
        } catch (e: Exception) {
            Resource.Error(e.message.toString())
        }
    }
}